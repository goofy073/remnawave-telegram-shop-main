...
func (h Handler) buildStartKeyboard(existingCustomer *database.Customer, langCode string) [][]models.InlineKeyboardButton {
	...
	// Add Instructions button if at least one instruction URL is configured
	if config.InstructionsTVURL() != "" || config.InstructionsAndroidURL() != "" || config.InstructionsIOSURL() != "" || config.InstructionsPCURL() != "" {
		inlineKeyboard = append(inlineKeyboard, []models.InlineKeyboardButton{{Text: h.translation.GetText(langCode, "instructions_button"), CallbackData: CallbackInstructions}})
	}
