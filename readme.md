...
- `/sync` - Poll users from remnawave and synchronize them with the database. Remove all users which not present in
  remnawave.
- `/broadcast <message>` - Send a message to all bot users. Supports HTML formatting. Shows progress and delivery statistics.
...
| `INSTRUCTIONS_TV_URL`    | URL to TV instructions page (optional) - if not set, TV button will not be displayed in Instructions menu                                   |
| `INSTRUCTIONS_ANDROID_URL` | URL to Android instructions page (optional) - if not set, Android button will not be displayed in Instructions menu                       |
| `INSTRUCTIONS_IOS_URL`   | URL to iOS instructions page (optional) - if not set, iOS button will not be displayed in Instructions menu                                 |
| `INSTRUCTIONS_PC_URL`    | URL to PC instructions page (optional) - if not set, PC button will not be displayed in Instructions menu                                   |
...
- Instructions button is displayed if at least one instruction URL (TV, Android, iOS, or PC) is configured
- The Instructions menu dynamically shows only the platforms that have URLs configured
